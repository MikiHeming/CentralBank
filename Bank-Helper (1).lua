...
function main()
	...
	sampRegisterChatCommand('kip', function()
		...
		lua_thread.create(function()
			...
			sampSetCursorMode(4)
			addBankMessage('Нажмите {M}ПРОБЕЛ{W} что-бы сохранить местоположение, или {M}ESC{W} что-бы отменить')
			while ChangePos == true do
				...
				if isKeyJustPressed(VK_SPACE) then
					ChangePos = false
					addBankMessage('Позиция сохранена!')
				elseif isKeyJustPressed(VK_ESCAPE) then
					ChangePos = false
					cfg.main.KipX = backup['x']
					cfg.main.KipY = backup['y']
					addBankMessage('Вы отменили изменение местоположения')
				end
			end
	...
	sampRegisterChatCommand('ro', function(text)
		if cfg.main.rank >= 9 or bratishka then
			...
			addBankMessage('Доступно только с 9 ранга')
		end
	end)
		...
		if sampIsChatInputActive() and chat_calc.v then
			...
			if example:match('[0-9]+') and example:match('[-+/*^]+') then
		        ...
		        if result then
			        ...
			        local output = string.format('Результат: {00AAFF}%s', res and format or answer)
	...
	if ki_stat.v then
		if text:find('Касса') and text:find(sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)))) then
			...
			if kassa['time'].v:match('^0') then 
				addBankMessage(string.format('Вы встали на пост: {M}%s{W}! Чтобы изменить местоположение панели информации введите {W}/kip', kassa['name'].v)) 
				kassa['money'] = 0
			end
			return false
		end
...
function se.onServerMessage(clr, msg) -- хук чата
	...
	local leader, rank = msg:match('^Лидер [A-z0-9_]+ повысил до %d+ ранга')
	...
	if msg:find('^Лидер вашей организации [A-z0-9_]+ повысил вас в должности') then
		cfg.main.dateuprank = os.time()
		await['get_rank'] = os.clock()
		await['rank_update'] = os.clock()
		send('/stats')
	...
	if msg:find(sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)))..' переодевается в гражданскую одежду') then
		addNotify('{006D86}Рабочий день окончен!\nУдачно отдохнуть!', 5)
		return false
	end

	if msg:find(sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)))..' переодевается в рабочую одежду') then
	...
	if member and client and reason then 
		if chat['expel'].v then
			msg = string.format(tag.. 'Сотрудник %s выгнал из банка %s по причине: %s', mc .. member .. wc, mc .. client .. wc, mc .. reason)
	...
	if sum and nick then 
		if chat['shtrafs'].v then
			msg = string.format(tag .. 'Казна банка пополнена на %s. Житель %s внёс оплату за штраф', mc .. sum .. wc, mc .. nick:gsub('_', ' ') .. wc)
			return { 0x006699FF, msg }
		end
		return false
	...
	if nick and sum then 
		if chat['incazna'].v then
			msg = string.format(tag .. 'Сотрудник %s пополнил казну банка на %s', mc .. nick:gsub('_', ' ') .. wc, mc .. sum .. wc)
			return { 0x006699FF, msg }
		end
		return false
	end
	...
	if member and leader then 
		if chat['invite'].v then
			msg = string.format(tag .. '%s - новый член нашей организации. Пригласил: %s', mc .. member:gsub('_', ' ') .. wc, mc .. leader:gsub('_', ' ') .. wc)
			return { 0x006699FF, msg }
		end
		return false
	end
	...
	if leader and member and reason then 
		if chat['uval'].v then
			msg = string.format(tag .. 'Руководитель %s уволил сотрудника %s по причине: %s', mc .. leader:gsub('_', ' ') .. wc, mc .. member:gsub('_', ' ') .. wc, wc .. reason)
			return { 0x006699FF, msg }
		end
		return false
	end
	...
	if msg:find('^%[R%]') and clr == 0x2DB043FF then
		if msg:find('%[Объявление%]') and {msg:find(cfg.nameRank[9], 1, true) or msg:find(cfg.nameRank[10], 1, true) or bratishka} then
...
function se.onShowDialog(dialogId, style, title, button1, button2, text) -- хук диалогов
	...
	if dialogId == 0 and isAnyAwaitExist then
		...
		if text:find('Вы успешно отправили предложение на смену пароля') then
			addNotify("{006D86}Ожидайте..\nПредложение отправлено", 5)
		...
		if text:find('Баланс Фракций') then
			text = text:gsub(
				'%- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- {73B461}Баланс Фракций{FFFFFF} %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %- %-\n',
				'{FFFFFF}- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - {73B461}Баланс Фракций{FFFFFF} - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n'  
			)
			text = separator(text)
			return { dialogId, style, title, button1, button2, text }
		end

		if text:find('PIN-код принят', 1, true) then
			send(1)
			addBankMessage('PIN-код принят!')
	...
	if dialogId == 228 and await['credit_send'] then
		addNotify("{006D86}Предложение отправлено\nДождитесь одобрения", 5)
	...
	if dialogId == 235 and await['get_rank'] then
		...
		for line in text:gmatch('[^\r\n]+') do
			local name, rank = line:match('^{%x+}Должность: {%x+}(.+)%((%d+)%)')
			if rank then
				rank = tonumber(rank)
				if rank ~= cfg.main.rank then
					...
					if inicfg.save(cfg, 'Bank_Config.ini') and await['rank_update'] then 
						...
						addBankMessage(string.format('Ваша должность обновлёна на {M}%s(%s)', name, rank))
					end
				elseif await['rank_update'] then
					await['rank_update'] = nil
					addBankMessage('Несоответствие вашей должности не обнаружено!')
				end
			end
		end
...
function imgui.OnDrawFrame()
	...
	if int_bank.v then
		...
		imgui.Begin(u8'##IntMenuBank', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoScrollbar)
			...
			if sampIsPlayerConnected(actionId) then
				...
				imgui.CenterTextColoredRGB(mc..'Лет в штате')
				local yearold = sampGetPlayerScore(actionId)
				if TypeAction.v ~= 2 then 
					imgui.CenterTextColoredRGB(yearold..' лет')
				else
					imgui.CenterTextColoredRGB(yearold..' лет | '..(yearold >= 3 and 'Подходит' or '{FF0000}Не подходит'))
				end
			else
				int_bank.v = false
			...
			if boolSkin then 
				imgui.CenterTextColoredRGB('Скин №'..id_skin)
				imgui.CenterTextColoredRGB(isSkinBad(id_skin) and '{FF0000}(Не приличный)' or '(Приличный)')
			else
				imgui.CenterTextColoredRGB('Скин не определён')
			end
			imgui.EndChild()
			imgui.SameLine()
			if TypeAction.v == 2 or TypeAction.v == 3 then
				...
				if TypeAction.v == 1 then
					if not go_credit.v then
						...
						if imgui.Button(u8('Приветствие ')..fa.ICON_FA_CHILD, imgui.ImVec2(200, 30)) then
							int_bank.v = false
							play_message(MsgDelay.v, false, {
								{ "/do На груди бейджик «%s»", cfg.nameRank[cfg.main.rank] },
								{ "Здравствуйте, я %s - {my_name}", cfg.nameRank[cfg.main.rank] },
								{ "Чем могу Вам помочь?" }
							})
						end
						imgui.SameLine()
						if cfg.main.rank > 1 then
							if imgui.Button(u8('Выгнать ')..fa.ICON_FA_BAN, imgui.ImVec2(-1, 30)) then
								imgui.OpenPopup("##edit_expel")
							end
						else
							imgui.DisableButton(u8('Выгнать ')..fa.ICON_FA_BAN, imgui.ImVec2(-1, 30))
							imgui.Hint('expel2rank', u8'Доступно с 2+ ранга')
						end
						if imgui.BeginPopupContextWindow("##edit_expel", 1, false) then
							imgui.PushItemWidth(150)
							imgui.Text(u8'Причина для /expel:')
							if imgui.InputText(u8'##ExpelReason', expelReason) then
								cfg.main.expelReason = u8:decode(expelReason.v)
							end
							imgui.PopItemWidth()
							if imgui.MainButton(u8'Выгнать игрока') then
								int_bank.v = false
						...
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Депозит: СНЯТЬ ')..fa.ICON_FA_MINUS_CIRCLE, imgui.ImVec2(250, 30)) then
								int_bank.v = false
							...
							imgui.DisableButton(u8('Депозит: СНЯТЬ ')..fa.ICON_FA_LOCK, imgui.ImVec2(250, 30))
							imgui.Hint('depositdown5rank', u8'Доступно с 5+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 80))
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Депозит: ПОПОЛНИТЬ ')..fa.ICON_FA_PLUS_CIRCLE, imgui.ImVec2(250, 30)) then
								int_bank.v = false
							...
							imgui.DisableButton(u8('Депозит: ПОПОЛНИТЬ ')..fa.ICON_FA_LOCK, imgui.ImVec2(250, 30))
							imgui.Hint('depositeup5rank', u8'Доступно с 5+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 115))
						if cfg.main.rank > 5 then
							if imgui.Button(u8('Выдать кредит ')..fa.ICON_FA_CALCULATOR, imgui.ImVec2(-1, 30)) then
								go_credit.v = true
								play_message(MsgDelay.v, false, {
							...
							imgui.DisableButton(u8('Выдать кредит ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('credit6rank', u8'Доступно с 6+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 150))
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Узнать долг банку ')..fa.ICON_FA_MONEY_CHECK, imgui.ImVec2(-1, 30)) then
								int_bank.v = false
								await['debt'] = os.clock()
								play_message(MsgDelay.v, false, {
							...
							imgui.DisableButton(u8('Узнать долг банку ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('debt5rank', u8'Доступно с 5+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 185))
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Узнать кол-во денег в банке ')..fa.ICON_FA_PIGGY_BANK, imgui.ImVec2(-1, 30)) then
								int_bank.v = false
							...
							imgui.DisableButton(u8('Узнать кол-во денег в банке ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('howmuchmoney5rank', u8'Доступно с 5+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 220))
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Выдать карту ')..fa.ICON_FA_CREDIT_CARD, imgui.ImVec2(-1, 30)) then
								int_bank.v = false
								await['card_create'] = os.clock()
								play_message(MsgDelay.v, false, {
									{ "/me {sex:открыл|открыла} на планшете раздел «Банковские Карты»" },
									{ "/me {sex:выбрал|выбрала} пункт «Создать бланк» и {sex:нажал|нажала} «Распечатать»" },
									{ "/me {sex:взял|взяла} распечатанный бланк и {sex:поставил|поставила} нужные печати" },
									{ "/todo Заполните этот бланк и карта ваша!*смотря на %s", rpNick(actionId) },
									{ "/bankmenu %s", actionId }
								})
							end
						else
							imgui.DisableButton(u8('Выдать карту ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('givecard5rank', u8'Доступно с 5+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 255))
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Восстановить карту ')..fa.ICON_FA_RECYCLE, imgui.ImVec2(-1, 30)) then
								imgui.OpenPopup(u8("Выбор действия##Card"))
							end
						else
							imgui.DisableButton(u8('Восстановить карту ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
						...
						if imgui.BeginPopupModal(u8("Выбор действия##Card"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
							...
							imgui.NewLine()
							if imgui.Button(u8('Написать в /report'), imgui.ImVec2(150, 30)) then
							...
							imgui.SameLine()
							if imgui.Button(u8('Пересоздать карту'), imgui.ImVec2(150, 30)) then
								imgui.CloseCurrentPopup()
								int_bank.v = false
								await['card_recreate'] = os.clock()
								play_message(MsgDelay.v, false, {
									{ "Обратите внимание, данная услуга обойдётся вам в 30.000$" },
									{ "/me {sex:открыл|открыла} на планшете раздел «Банковские Карты»" },
									{ "/me {sex:выбрал|выбрала} пункт «Перевыдача карт» и {sex:нажал|нажала} «Распечатать»" },
									{ "/me {sex:взял|взяла} распечатанный бланк и {sex:поставил|поставила} нужные печати" },
									{ "/todo Поставьте подписи там где галочки и карта ваша*передовая бланк %s", rpNick(actionId) },
									{ "/bankmenu %s", actionId }
								})
							end
							if imgui.MainButton(u8('Назад'), imgui.ImVec2(-1, 30)) then
								imgui.CloseCurrentPopup()
							end
							imgui.EndPopup()
						end
				...
				if TypeAction.v == 2 then
					...
					imgui.CenterTextColoredRGB('• 3 года проживания в штате (3+ лвл)')
					...
					imgui.CenterTextColoredRGB('• Адекватное поведение')
					imgui.NewLine()
					...
					if imgui.Button(u8('Паспорт'), imgui.ImVec2(88, 30)) then
						play_message(MsgDelay.v, false, {
					...
					imgui.SameLine()
					if imgui.Button(u8('Мед. карта'), imgui.ImVec2(88, 30)) then
						play_message(MsgDelay.v, false, {
					...
					imgui.SameLine()
					if imgui.Button(u8('Лицензии'), imgui.ImVec2(-1, 30)) then
						play_message(MsgDelay.v, false, {
					...
					if question.v == 1 then 
						if imgui.Button(u8('Мини-Опрос [Вопрос №1]'), imgui.ImVec2(-1, 30)) then
							question.v = 2
							play_message(MsgDelay.v, false, {
								{ "Так, теперь пару вопросиков для вас.." },
								{ "Будете ли выполнять свои должностные обязанности на работе?" }
							})
						end
					elseif question.v == 2 then 
						if imgui.Button(u8('Мини-Опрос [Вопрос №2]'), imgui.ImVec2(-1, 30)) then
							question.v = 3
							play_message(MsgDelay.v, false, {
					...
					elseif question.v == 3 then 
						if imgui.Button(u8('Мини-Опрос [Вопрос №3]'), imgui.ImVec2(-1, 30)) then
							question.v = 1
							addBankMessage('Вопросы кончились! Можно завершать опрос!')
							play_message(MsgDelay.v, false, {
					...
					if imgui.Button(u8('Принять ')..fa.ICON_FA_USER_PLUS, imgui.ImVec2(135, 30)) then
					...
					if imgui.Button(u8('Отклонить ')..fa.ICON_FA_USER_TIMES, imgui.ImVec2(-1, 30)) then
	...
	if bank.v then
		...
		if type_window.v == 1 then -- Бинды
			if #cfg.Binds_Name > 0 then
				...
				imgui.PopFont()
				imgui.NewLine()
				imgui.CenterTextColoredRGB('Бинды для работы с клиентами уже есть в скрипте!\nДля их использования нажмите:\nПКМ + Q (наведясь на игрока)')
				imgui.NewLine()
				imgui.SetCursorPosX((imgui.GetWindowWidth() - 130) / 2)
				if imgui.Button(u8('Создать свой бинд!'), imgui.ImVec2(130, 30)) then
					imgui.OpenPopup(u8("Редактирование/Создание бинда"))
					binder_open = true
				end
			end
		...
		if type_window.v == 5 then -- Настройки
			...
			if imgui.CollapsingHeader(u8("Общие настройки")) then
				...
				imgui.SameLine()
				imgui.TextDisabled('(?)')
				imgui.Hint('chatcalc', u8'Если в поле ввода написать пример, то под ним появится ответ')
				imgui.PopItemWidth()
				if imgui.Checkbox(u8'Статистика на кассе', ki_stat) then
					if cfg.main.rank < 5 then 
						ki_stat.v = false
						addBankMessage('Доступно с 5 ранга!') 
					else
						addBankMessage(string.format('Статистика на кассе: {M}%s', (ki_stat.v and mc..'Включена' or mc..'Выключена')))
			...
			if imgui.CollapsingHeader(u8("Настройки цветов")) then
				...
				if bratishka then 
					if imgui.ColorEdit4(u8'Цвет ника в чате орг', colorCosmo, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoAlpha) then
					...
					imgui.SameLine()
					imgui.TextDisabled('(?)')
					imgui.Hint('colororgchat', u8'Изменённый цвет будете видеть только вы!\nОстальные будут видеть бирюзовый цвет')
					imgui.SameLine(imgui.GetWindowWidth() - 150)
					if imgui.Button(u8("Тест##CosmoCol"), imgui.ImVec2(50, 20)) then
						local cr, cg, cb, ca = imgui.ImColor(cfg.main.colorCosmo):GetRGBA()
						local mr, mg, mb, ma = imgui.ImColor(cfg.main.colorRchat):GetRGBA()
						sampAddChatMessage(string.format('[R] '..cfg.nameRank[cfg.main.rank]..' {%06X}'..sampGetPlayerNickname(tonumber(selfid))..'['..selfid..']{%06X}:(( Это сообщение видите только вы! ))', join_rgb(cr, cg, cb), join_rgb(mr, mg, mb)), join_rgb(mr, mg, mb))
					end
					imgui.SameLine(imgui.GetWindowWidth() - 95)
					if imgui.Button(u8("Стандартный##CosmoCol"), imgui.ImVec2(90, 20)) then
						cfg.main.colorCosmo = 4294940979
						if inicfg.save(cfg, 'Bank_Config.ini') then 
							addBankMessage('Стандартный цвет ника в чате орг. восстановлен!')
							colorCosmo = imgui.ImFloat4(imgui.ImColor(cfg.main.colorCosmo):GetFloat4())
						end
					end
				else
					imgui.ColorEdit4(u8'##Цвет ника Disabled', colorCosmo, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoPicker + imgui.ColorEditFlags.NoTooltip)
					if imgui.IsItemClicked() then addBankMessage('Вам не доступна эта функция!') end
					imgui.SameLine(32)
					imgui.TextDisabled(u8('Цвет ника ')..fa.ICON_FA_LOCK)
					imgui.Hint('colormynick', u8'Функция доступна пользователям с фамилией Cosmo\nНапример: Bob_Cosmo')
					imgui.SameLine(imgui.GetWindowWidth() - 150)
					imgui.DisableButton(u8"Тест##CosmoColDisabled", imgui.ImVec2(50, 20))
					imgui.SameLine(imgui.GetWindowWidth() - 95)
					imgui.DisableButton(u8"Стандартный##CosmoColDisabled", imgui.ImVec2(90, 20))
				end
			end
			...
			if imgui.CollapsingHeader(u8("О скрипте")) then
				...
				if imgui.Button(fa.ICON_FA_LINK..u8(" ВКонтакте"), imgui.ImVec2(90, 25)) then
					addBankMessage('Ссылка скопирована в буфер обмена')
	...
	if ustav_window.v then
		...
		imgui.Begin(u8'Устав Центрального Банка', ustav_window, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar)
			...
			imgui.Separator()
			local ustav = io.open("moonloader/BHelper/Устав ЦБ.txt", "r+")
		...
		imgui.Begin(u8("Статистика"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoBringToFrontOnFocus + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.NoTitleBar)
			...
			imgui.BeginGroup()
				...
				if distBetweenKassa < 10 then
					...
					imgui.Text(fa.ICON_FA_CHECK_CIRCLE);    imgui.SameLine(30); imgui.TextColoredRGB(mc..'Оформлено депозитов: {SSSSSS}'    ..tostring(cfg.kassa.dep))
					imgui.Text(fa.ICON_FA_HANDSHAKE);       imgui.SameLine(30); imgui.TextColoredRGB(mc..'Оформлено кредитов: {SSSSSS}'     ..tostring(cfg.kassa.credit))
					imgui.Text(fa.ICON_FA_CREDIT_CARD);     imgui.SameLine(30); imgui.TextColoredRGB(mc..'Выдано карт: {SSSSSS}'            ..tostring(cfg.kassa.card))
					imgui.Text(fa.ICON_FA_RECYCLE);         imgui.SameLine(30); imgui.TextColoredRGB(mc..'Восстановлено карт: {SSSSSS}'     ..tostring(cfg.kassa.recard))
					imgui.Text(fa.ICON_FA_PIGGY_BANK);      imgui.SameLine(30); imgui.TextColoredRGB(mc..'Заработано: {SSSSSS}'             ..kassa['money']..'$')
				else
					imgui.CenterTextColoredRGB(sc..'Пост: '..kassa['name'].v:gsub('\n', ''))
...
function go_expel(playerId)
	...
	if target_color ~= self_color then
		if not sampIsPlayerPaused(playerId) then
			play_message(MsgDelay.v, true, {
				{ "Мне придётся вывести вас из здания банка." },
				{ "/me {sex:схватил|схватила} за руку %s и {sex:повёл|повела} к выходу", rpNick(playerId) },
				{ "/todo А теперь подумайте над своим поведением!*закрывая дверь банка.." },
...
function onScriptTerminate(script, quitGame)
	if script == thisScript() then
		...
		if not noErrorDialog and not devmode then
			sampShowDialog(1313, "{006AC2}[ Bank ] Скрипт завершил свою работу", [[
{006AC2}                                                           Что делать если не работает скрипт?{FFC973}
...
2. Если вы установили и запустили скрипт впервые, то скорее всего он просто скачал
 нужные ему библиотеки. От вас лишь требуется нажать CTRL + R, в 90 процентах случаях
 это помогает
...
3. Если у вас установлен MVD Helper, то вам нужно его удалить, он использует старую
 ...
 Удалите MVD Helper и повторно установите Bank Helper
...
- В папке moonloader > Удаляем папку BHelper

7. Если данные действия не помогли, то попробуйте установить скрипт на другую сборку

{00FFA5}Все файлы для скрипта можно найти по ссылке, которая {33AA33}скопирована в буфер обмена.

{FFFFFF}Попытаться восстановить работу скрипта можно сочетанием  клавиш CTRL + R]], "Понял", _, 0)
...
function system_credit()
	if imgui.BeginPopupModal(u8("Система кредитования"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
		...
		credits:close()
		imgui.Separator()
		imgui.CenterTextColoredRGB(mc..'Вы можете изменить на свою! {464646}> Узнать как <')
...
function BlackListGui()
	if imgui.BeginPopupModal(u8("Чёрный список правительства"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.AlwaysAutoResize) then
		imgui.CenterText(u8('Игроков состоящих в этих списках\ncкрипт не даст вам принять в организацию'))
		if #cfg.blacklist > 0 then
			imgui.CenterText(u8('Что бы удалить - кликните нику дважды'), imgui.ImVec4(1.0, 0.5, 0.5, 0.7))
		end
		imgui.BeginChild("##BlakList", imgui.ImVec2(300, 250), true, imgui.WindowFlags.NoScrollbar)
		imgui.PushFont(font[15])
		if #cfg.blacklist == 0 then
			imgui.SetCursorPosY(110)
			imgui.CenterText(u8'Список пуст', imgui.ImVec4(1.0, 1.0, 1.0, 0.3))
		end
		...
		if imgui.Button(u8("Сохранить и выйти##BlackList"), imgui.ImVec2(-1, 25)) then
			inicfg.save(cfg, 'Bank_Config.ini')
...
function binder()
	if imgui.BeginPopupModal(u8("Редактирование/Создание бинда"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
		imgui.BeginChild("##EditBinder", imgui.ImVec2(500, 355), true)
			imgui.PushItemWidth(150)
			imgui.InputInt(u8("Задержка между строками в миллисекундах"), binder_delay); imgui.SameLine(); imgui.TextDisabled('(?)')
			imgui.Hint('maxdelaybinder', u8('Не больше 60.000ms!\n(1 sec = 1000 ms)'))
			imgui.PopItemWidth()
	...
	if cmd:match('^/premium %d+ %d+ %d+') and cfg.main.rank >= 9 then
		...
		if rank then
			...
			if cfg.nameRank[rank] ~= nil and temp_prem ~= cfg.nameRank[rank] then
				...
				sampSendChat('Премия для сотрудников на должности ' .. temp_prem)
				sampSendChat(cmd)
				return false
			end
		end
...
function checkServer(ip)
	...
	for i, v in pairs(servers) do
		if v == ip then
			log("Проверка на сервер: {33AA33}Успешно [ №" .. i .. " ]", "Подготовка")
			return true
		end
	end
	log("Проверка на сервер: {FF1010}Неудачно [ " .. ip .. " ]", "Подготовка")
	return false
end
...
function helpCommands()
	if imgui.BeginPopupModal(u8("Все команды скрипта"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) then
		...
		imgui.BeginGroup()
			...
			imgui.TextColoredRGB(mc..'/ro [text]{SSSSSS} - РП чат объявлений (9+ ранг)')
			imgui.TextColoredRGB(mc..'/rbo [text]{SSSSSS} - НонРП чат объявлений (9+ ранг)')
		...
		imgui.BeginGroup()
			...
			imgui.TextColoredRGB(mc..'ПКМ + G{SSSSSS} - Выгнать из банка с причиной Н.П.Б')
...
changelog = {
	...
	[12] = {
		...
		log = {
			...
			'Немного улучшен диалог состояний казны других организаций. Цифры в нём отображаются с запятыми (Например: 1.000.000 вместо 1000000)',
